...
rollButton.addEventListener(`click`, async function(event){
  ...
  let die1image = document.querySelector(`.die1`)
  
  die1image.setAttribute(`src`,`../images/dice/${die1}.png`)
  

  // - Replace the second die image using the second random value (hint: setAttribute)
  let die2image = document.querySelector(`.die2`)
  
  die2image.setAttribute(`src`,`../images/dice/${die2}.png`)
  ...
  resultSection.insertAdjacentHTML (`beforeend`,`<li>${sentence}</li>`)
